...
VERSION = edge
TAG = $(VERSION)
PREFIX = feifeiiiiiiiiiii/mini-nginx-ingress

DOCKER_RUN = docker run --rm -v $(shell pwd):/go/src/github.com/feifeiiiiiiiiiii/mini-ingress-nginx
DOCKER_BUILD_RUN = docker run --rm -v $(shell pwd):/go/src/github.com/feifeiiiiiiiiiii/mini-ingress-nginx -w /go/src/github.com/feifeiiiiiiiiiii/mini-ingress-nginx/cmd/nginx/
